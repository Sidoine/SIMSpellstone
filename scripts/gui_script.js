...
window.addEventListener("error", function (message, url, linenumber) {
    ...
    if (getbattleground)
        err_msg += "Battleground ID: " + getbattleground + "\n";
    ...
    outp(
        '<br><br><i>Error Message:</i><br><textarea cols=50 rows=6 onclick="this.select()"><blockquote>' +
            err_msg +
            "</blockquote></textarea>" +
            echo
    );
...
function processQueryString() {
    ...
    if (missionID) {
        if (!campaignID) {
            campaignID = Object.keys(CAMPAIGNS).filter(function isCampaign(
                campaignID
            ) {
    ...
    document.title =
        "SimSpellstone " +
        text_version +
        " - The Spellstone Simulator that runs from your browser!";
...
function outputTurns(turnData, showAll) {
    ...
    turnData =
        "<input id='show-turns' type='button' value='Show All' /> <div id='turn-container'>Turn: <select id='turn-picker'></select></div> <div>" +
        turnData +
        "</div>";
    outp(turnData);
...
function generate_link(autostart) {
    ...
    var mapBges = $("select[name=map-battleground]")
        .toArray()
...
function open_deck_builder(name, hash, inventory, deckHashField) {
    ...
    $(win).load(
        (function (name, deckHashField) {
            return function () {
                // Tie deck-builder back to the hash field in the simulator.
                if (deckHashField)
                    win.updateSimulator = function (hash) {
                        deckHashField.val(hash).change();
                    };
            };
        })(name, deckHashField)
    );
